...
"use strict";
    ...
    this.load_bin = function(fileROM) {
        ...
        function putInMemory(filestream) {
            ...
            for (let n = 0; n < filestream.length; n++) {
                Memory._memory[n] = filestream.charCodeAt(n) & 0xff; // Throw away high-order byte
            }

            //bck_memory = memory;
            Memory._bck_memory = Memory._memory;
    ...
    this.check_content_is_zip = function(content) {
    ...
    this.get_zip_files = function(zip) {
        //var files = zip.file(/.+/);
        //let files = zip.file(/.+/);
        //return zip.file(/.+/);/file/
        let regex = /.+/
        return zip.file(regex);   // files
    };
