...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const core = {
        ...
        io_read   : function(port)     { return io_read  (port)    ; }, //io_read   : function(port)     { return my_io.read  (port)    ; },
        io_write  : function(port,val) {  io_write (port,val); }, //io_write  : function(port,val) {        my_io.write (port,val); },
    };

    let cpu = new Z80(core); //let cpu = new I8080(core);
    ...
    function reset() {
        cpu.iff = 0;
        cpu.reset; //cpu.jump(0xf800); // не работает
        video.set_mode(0); //++
        keyboard.reset();
    }
    ...
    let io_read = function(addr) { //cpu.readIo = function(addr) {
        let result;
        switch (addr & 0xff) {
            case 0x9e:
                result = spi_sd.read(0)
                break;
            case 0x9f:
                result = spi_sd.read(1)
                break;
            default:
    ...
    let io_write = function(addr, byte) { //cpu.writeIo = function(addr, byte) {
        switch (addr & 0xff) { //switch (addr & 0xff00) {
            case 0x9e:
                spi_sd.write(0, byte)
                break;
            case 0x9f:
    ...
    function cpuTick() {
        ...
        while (tickCount < needTickCount) {
            ...
            const t = cpu.run_instruction(); //const t = cpu.instruction(false);
            tickCount += t;
            audio.audioTick += t;
